...
public class RPCLiteralType
{
    ...
    private static Element buildSchemaElement(final PGAppObj pgAppObj) {
        ...
        try {
            ...
            if (pgAppObj.hasParamType(17, false)) {
                element.appendChild(buildGenericTableHandleElement(wsdlGenInfo));
            }
            ...
            for (int i = 0; i < pgAppObj.getProcedures().length; ++i) {
                ...
                buildParamElementsForProc(procedures.getProcDetail(), element, false, wsdlGenInfo);
            ...
            for (int j = 0; j < pgAppObj.getPersistentProcedures().length; ++j) {
                ...
                if (!pgAppObj.containsProc(persistentProcedures)) {
                    buildParamElementsForProc(persistentProcedures.getProcDetail(), element, false, wsdlGenInfo);
    ...
    private static Element buildSchemaElement(final PGProc pgProc) {
        ...
        try {
            ...
            element.setAttribute("xmlns", wsdlGenInfo.getXSDSchemaNamespace());
            element.setAttribute("elementFormDefault", "unqualified");
            element.appendChild(buildObjectIDElement(pgProc.getProcDetail().getMethodName(), wsdlGenInfo));
            ...
            if (pgProc.hasParamType(15, true)) {
                ...
                for (int i = 0; i < internalProcs.length; ++i) {
                    ...
                    if (!pgMethod.isExcluded()) {
                        buildParamElementsForProc(pgMethod.getMethodDetail(), element, true, wsdlGenInfo);
                    }
                }
    ...
    private static void buildParamElementsForProc(final IPGDetail ipgDetail, final Element element, final boolean b, final WSDLGenInfo wsdlGenInfo) {
    ...
    private static Element buildGenericTableHandleElement(final WSDLGenInfo wsdlGenInfo) {
        final CoreDocumentImpl coreDocImpl = wsdlGenInfo.getCoreDocImpl();
        Element element = null;
        try {
            element = coreDocImpl.createElement("complexType");
    ...
    private static Element buildArrayElement(final PGParam pgParam, final WSDLGenInfo wsdlGenInfo) {
        ...
        final String proToXMLType = WSDLDataTypes.proToXMLType(paramType);
        final String proToXMLArrayType = WSDLDataTypes.proToXMLArrayType(paramType);
        try {
            element = coreDocImpl.createElement("complexType");
            ...
            element3.setAttribute("type", "xsd:" + proToXMLType);
            element3.setAttribute("minOccurs", "0");
            element3.setAttribute("maxOccurs", "unbounded");
            element2.appendChild(element3);
            element.appendChild(element2);
