...
public class EsbDecimal
{
    ...
    static String serializeParameter(final Object o, final String s, final int n) throws ScriptEngineException {
        if (n != 5) {
            throw new ScriptEngineException("Incorrect parameter type - Expected DECIMAL, got " + Parameter.proToName(n), 0);
        }
        return o.toString();
    }
    ...
    static String serializeArrayParameter(final Object o, final String s, final int n, final int n2) throws ScriptEngineException {
        ...
        final BigDecimal[] array = (BigDecimal[])o;
        for (int i = 0; i < array.length; ++i) {
            final BigDecimal bigDecimal = array[i];
            if (null != bigDecimal) {
                sb.append("<item>");
    ...
    static Object parseParameter(final String s, final int n) throws ScriptEngineException {
        ...
        try {
            ...
            throw new ScriptEngineException("Invalid DECIMAL value '" + s + "' sent to deserializer", 1);
        }
        return bigDecimal;
    }
    
    static Object[] parseArrayParameter(final Element element, final int n, final int n2) throws ScriptEngineException {
        final ArrayList<BigDecimal> list = new ArrayList<BigDecimal>();
        if (n != 5) {
            throw new ScriptEngineException("Incorrect parameter type - Expected DECIMAL, got " + Parameter.proToName(n), 0);
        }
        ...
        while (iterator.hasNext()) {
            ...
            BigDecimal e;
            try {
                e = new BigDecimal(firstTextChildValue);
            }
            catch (NumberFormatException ex) {
        ...
        final BigDecimal[] a = new BigDecimal[list.size()];
        list.toArray(a);
        return a;
    }
    
    static {
        EsbDecimal.m_log = XQLogImpl.getCategoryLog(64);
    }
}
