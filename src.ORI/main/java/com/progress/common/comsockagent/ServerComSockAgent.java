...
public class ServerComSockAgent extends Thread
{
    boolean timeToGo;
    private int port;
    ...
    public ServerComSockAgent(final int n) {
        this(n, null);
    }
    
    public ServerComSockAgent(final int n, final InetAddress inetAddr) {
        this.timeToGo = false;
    ...
    public void send(final int i, final ComMsgAgent comMsgAgent) throws IOException {
        synchronized (this.clients) {
            for (int size = this.clients.size(), j = 0; j < size; ++j) {
                final Client client = this.clients.elementAt(j);
                if (client.id == i) {
                    comMsgAgent.write(client.outStream);
                    return;
                }
            }
        }
    ...
    public void addEventListener(final ServerComSockAgentListener serverComSockAgentListener) {
        if (!this.listeners.contains(serverComSockAgentListener)) {
            this.listeners.addElement(serverComSockAgentListener);
        }
    }
            ...
            try {
                do {
                    ...
                    for (int size = this.listeners.size(), i = 0; i < size; ++i) {
                        ((ServerComSockAgentListener)this.listeners.elementAt(i)).clientConnect(obj.id);
                    }
                    obj.start();
                } while (!this.timeToGo);
            }
