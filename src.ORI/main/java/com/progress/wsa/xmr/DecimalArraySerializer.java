...
public class DecimalArraySerializer implements Deserializer
{
    public Bean unmarshall(final String s, final QName qName, final Node node, final XMLJavaMappingRegistry xmlJavaMappingRegistry, final SOAPContext soapContext) throws IllegalArgumentException {
        final Vector unMarshall = new ArrayParamSerializer().unMarshall(s, new QName("http://www.w3.org/2001/XMLSchema", "decimalPrgs"), xmlJavaMappingRegistry, soapContext, (Element)node, new BigDecimal(0));
        return new Bean(BigDecimal[].class, (Object)unMarshall.toArray(new BigDecimal[unMarshall.size()]));
    }
}
