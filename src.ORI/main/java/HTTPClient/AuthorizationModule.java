...
class AuthorizationModule implements HTTPClientModule
{
    ...
    public int requestHandler(final Request req, final Response[] resp) throws IOException, AuthSchemeNotImplException {
        ...
        Label_0421: {
            if (con.getProxyHost() != null && !this.prxy_from_4xx) {
                ...
                AuthorizationInfo guess = proxy_auth_list.get(con.getProxyHost() + ":" + con.getProxyPort());
                if (guess == null) {
                    guess = AuthorizationInfo.getAuthorization(con.getProxyHost(), con.getProxyPort(), "Basic", "", con.getContext());
                }
                if (guess != null) {
            ...
            if (!this.auth_from_4xx) {
                ...
                if (guess != null) {
                    if (auth_handler != null) {
                        ...
                        catch (AuthSchemeNotImplException asnie2) {
                            break Label_0655;
                        }
                        if (guess == null) {
                            break Label_0655;
    ...
    private void handle_auth_challenge(final Request req, final Response resp) throws AuthSchemeNotImplException, IOException {
        ...
        if (this.prxy_sent != null) {
            final HTTPConnection con = req.getConnection();
            Util.getList(AuthorizationModule.proxy_cntxt_list, con.getContext()).put(con.getProxyHost() + ":" + con.getProxyPort(), this.prxy_sent);
        }
